...
def safe_transform_pts(bbox2d_crop, tf_to_crops):
    with transform_lock:
        inverted_tf = tf_to_crops.inverse()
        result = transform_pts(bbox2d_crop, inverted_tf).reshape(-1, 4)
    return result
...
def make_crop_data_batch(render_size, ob_in_cams, mesh, rgb, depth, K, crop_ratio, xyz_map, normal_map=None, mesh_diameter=None, cfg=None, glctx=None, mesh_tensors=None, dataset:PoseRefinePairH5Dataset=None):
  ...
  with transform_lock:
    inverted_tf = tf_to_crops.inverse()
    bbox2d_ori = transform_pts(bbox2d_crop, inverted_tf).reshape(-1,4)
